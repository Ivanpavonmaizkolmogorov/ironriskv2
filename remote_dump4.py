...
try:
    ...
    rows = db.execute(text("SELECT id, user_id, api_token, account_number, last_heartbeat_at, created_at FROM trading_accounts ORDER BY created_at DESC LIMIT 3")).fetchall()
    print("ALL ACCOUNTS:")
    for row in rows:
        print(row)
