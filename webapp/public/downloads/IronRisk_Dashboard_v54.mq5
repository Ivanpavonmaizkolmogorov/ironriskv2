   ...
   if(HistorySelect(0, TimeCurrent()))
     {
      ...
      // ANCHOR PREVENTATIVE MEASURE:
      // AppServer.SyncClosedDeals() calls HistorySelect(lastSync, TimeCurrent()), which restricts 
      // the terminal's global deal cache context. This causes the loop above to temporarily 
      // see 0 deals, crashing g_CumulativeClosedPnL to $0.00 and causing massive chart volatility.
      // We protect against this by ONLY accepting the new value if we found AT LEAST 
      // as many historical closed trades as we already knew existed.
