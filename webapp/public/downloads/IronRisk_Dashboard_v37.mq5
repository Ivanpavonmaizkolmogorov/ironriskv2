...
string CServerClient::DownloadRiskChart(long magic, string metric_name, double val)
  {
   ...
   int hC = InternetConnectW(hI, m_host, m_port, "", "", INTERNET_SERVICE_HTTP, 0, 0);
   if(!hC) { InternetCloseHandle(hI); return ""; }
   uint fl = INTERNET_FLAG_RELOAD | INTERNET_FLAG_DONT_CACHE; if(m_https) fl |= INTERNET_FLAG_SECURE;
   int hR = HttpOpenRequestW(hC, "GET", ep, "HTTP/1.1", NULL, NULL, (int)fl, 0);
   if(!hR) { InternetCloseHandle(hC); InternetCloseHandle(hI); return ""; }
   
   string finalName = "";
   bool res = HttpSendRequestW(hR, "", 0, NULL, 0);
   if(res)
     {
      ...
      int hFile = FileOpen(fileName, FILE_WRITE|FILE_BIN|FILE_COMMON);
      if(hFile == INVALID_HANDLE)
        {
         // Try local instead of common
         hFile = FileOpen(fileName, FILE_WRITE|FILE_BIN);
        }
        
      if(hFile != INVALID_HANDLE)
        {
         uchar buf[4096]; int rd=0;
     ...
     for(int i=0; i<g_RiskVarCount; i++)
      {
       ...
       LB(pfx+"_LBL", rvX+10, 72, g_RiskVars[i].label, 8, clrGray);
        ...
        bool hasCtx = (g_RiskVars[i].ctx_label != "" || g_RiskVars[i].ctx_percentile >= 0);
         int pctUsage = (g_RiskVars[i].limit > 0) ? (int)MathRound(current / g_RiskVars[i].limit * 100.0) : 0;
         string pctStr = "  " + IntegerToString(pctUsage) + "%";
         if(g_RiskVars[i].key=="max_drawdown" || g_RiskVars[i].key=="daily_loss")
           {
            valStr = DoubleToString(current, 2) + " / " + DoubleToString(g_RiskVars[i].limit, 2) + " $" + pctStr;
         ...
         else
           {
            ...
            valStr = IntegerToString(cur_int) + " / " + IntegerToString(lim_int) + pctStr;
           }
       ...
       // Percentage text
       string pctLabel = "";
       if(hasCtx && g_RiskVars[i].ctx_percentile >= 0) pctLabel = "P" + IntegerToString(g_RiskVars[i].ctx_percentile);
       else pctLabel = DoubleToString(pct * 100, 0) + "%";
       ...
       ObjectSetInteger(0, cName, OBJPROP_XDISTANCE, rvX + cardW_r - (g_RiskVars[i].key=="max_drawdown" ? 48 : 25));
       ObjectSetInteger(0, cName, OBJPROP_YDISTANCE, 70);
       ObjectSetInteger(0, cName, OBJPROP_XSIZE, 16);
       ObjectSetInteger(0, cName, OBJPROP_YSIZE, 16);
       ObjectSetInteger(0, cName, OBJPROP_FONTSIZE, 8);
       bool isChActive = (g_ActiveChartMetric == g_RiskVars[i].key);
       ObjectSetInteger(0, cName, OBJPROP_BGCOLOR, isChActive ? clrDodgerBlue : C'30,30,30');
       ObjectSetInteger(0, cName, OBJPROP_COLOR, isChActive ? clrWhite : clrGray);
       ...
       if(g_RiskVars[i].key=="max_drawdown")
         {
          ...
          ObjectSetInteger(0, eyeName, OBJPROP_XSIZE, 16);
          ...
          ObjectSetString(0, eyeName, OBJPROP_TEXT, "F"); // Floor
      ...
      if(StringFind(sparam, "IR_CHART_") == 0 && sparam != "IR_CHART_POPUP")
        {
         ...
         if(idx >= 0 && idx < g_RiskVarCount)
           {
            if(g_ActiveChartMetric == g_RiskVars[idx].key)
              {
               // Toggle off
               g_ActiveChartMetric = "";
              ...
              {
               // Toggle on
               g_ActiveChartMetric = g_RiskVars[idx].key;
               g_ChartMetricCardId = idx;
               // Download fresh chart from backend
               if(AppServer != NULL)
                 {
                  string fName = AppServer.DownloadRiskChart(g_ActiveMagic, g_ActiveChartMetric, g_RiskVars[idx].current);
