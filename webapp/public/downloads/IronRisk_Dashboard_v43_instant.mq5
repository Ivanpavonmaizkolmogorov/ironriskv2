...
//+------------------------------------------------------------------+
double g_LastFedPnL = -999999.9; // Memory of last value

void FeedPnLUpdate(double val)
  {
   datetime now = TimeCurrent();
   
   // 1. Feed tick for live line display ONLY IF VALUE CHANGED (Avoid nervous ticks)
   if(MathAbs(val - g_LastFedPnL) > 0.001)
     {
      MqlTick t[1];
      ZeroMemory(t[0]);
      t[0].time     = now;
...
void OnTimer()
  {
   ...
   if(tcLayout>=30) { tcLayout=0; RefreshLayout(); }
  }
      ...
      if(ci>=0 && ci<g_TotalStrategies)
        {
         ...
         // 2. Local-only recalculation and redraw (zero lag)
         // We DELIBERATELY skip sending heartbeat and fetching layout here to avoid synchronous WinINet UI freezing.
         // Fresh risk config/layout will arrive naturally via OnTimer() in <= 5 seconds.
         CalculateLocalStats();
         SeedHistoricalEquity();
         FeedPnLUpdate(g_StrategyEquity);
         DrawDashboard();
