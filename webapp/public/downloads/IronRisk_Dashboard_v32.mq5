...
void InitDefaultRiskVars()
  {
   g_RiskVarCount = 5;
   g_RiskVars[0].key="max_drawdown";      g_RiskVars[0].label="Max DD";          g_RiskVars[0].enabled=true;  g_RiskVars[0].limit=0; g_RiskVars[0].current=0;
   g_RiskVars[1].key="daily_loss";        g_RiskVars[1].label="Daily Loss";      g_RiskVars[1].enabled=true;  g_RiskVars[1].limit=0; g_RiskVars[1].current=0;
   g_RiskVars[2].key="consecutive_losses";g_RiskVars[2].label="Consec. Losses";  g_RiskVars[2].enabled=false; g_RiskVars[2].limit=0; g_RiskVars[2].current=0;
   g_RiskVars[3].key="stagnation_days";   g_RiskVars[3].label="Stagn. Days";     g_RiskVars[3].enabled=false; g_RiskVars[3].limit=0; g_RiskVars[3].current=0;
   g_RiskVars[4].key="stagnation_trades"; g_RiskVars[4].label="Stagn. Trades";   g_RiskVars[4].enabled=false; g_RiskVars[4].limit=0; g_RiskVars[4].current=0;
  }
...
void DrawDashboard()
  {
   ...
   LB("IR_TITLE", 20, 12, "IRONRISK DASHBOARD  v27", 16, clrWhite);
   color sc = clrGray;
   ...
   ObjectSetInteger(0,"IR_TLOG_BTN",OBJPROP_STATE,false);
    ...
    ObjectsDeleteAll(0, "IR_RV_");
     ...
     int rvStartX = 520;
     int cardW_r = 160; int cardGap = 8; int arrowW_r = 28;
     ...
     for(int i=0; i<g_RiskVarCount; i++)
      {
       ...
       LB(pfx+"_LBL", rvX+10, 72, g_RiskVars[i].label, 8, clrGray);
        ...
        LB(pfx+"_VAL", rvX+10, 86, valStr, 12, valCol);
        // Progress bar
       ...
       // Percentage text
       string pctStr = DoubleToString(pct * 100, 0) + "%";
       color pctCol = (pct > 0.7) ? clrWhite : clrGray;
