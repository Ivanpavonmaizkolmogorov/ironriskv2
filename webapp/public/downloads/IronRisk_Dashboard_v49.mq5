...
void FeedPnLUpdate(double val)
  {
   ...
   // 1. Smart Tick Injection: Only feed CustomTicksAdd if price changes, a new bar starts, 
   ...
   if(priceChanged || is_new_bar || forceKeepalive || g_LastFedPnL == EMPTY_VALUE)
     {
      MqlTick t[1];
      ZeroMemory(t[0]);
      t[0].time     = now;
...
void DrawDashboard()
  {
   ...
   LB("IR_TITLE", 20, 12, "IRONRISK DASHBOARD  v49", 16, clrWhite);
   color sc = clrGray;
