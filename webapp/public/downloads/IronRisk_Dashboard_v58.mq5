...
//+------------------------------------------------------------------+
//| OOP Metric Formatter                                             |
//+------------------------------------------------------------------+
class CMetricFormatter
  {
private:
   struct SFormatRule { string key; string type; };
   SFormatRule m_rules[];
   int m_count;
   
public:
   CMetricFormatter() { m_count = 0; RegisterDefaults(); }
   ~CMetricFormatter() { ArrayResize(m_rules, 0); m_count = 0; }
   
   void Register(string key, string type)
     {
      ArrayResize(m_rules, m_count + 1);
      m_rules[m_count].key = key;
      m_rules[m_count].type = type;
      m_count++;
     }
     
   void RegisterDefaults()
     {
      Register("max_drawdown", "currency");
      Register("daily_loss", "currency");
      Register("consecutive_losses", "integer");
      Register("stagnation_days", "integer");
      Register("stagnation_trades", "integer");
      Register("expected_payoff", "currency");
      Register("current_pnl", "currency");
      Register("net_profit", "currency");
      Register("total_trades", "integer");
      Register("volume", "decimal");
     }
     
   string FormatWithSpaces(string numStr)
     {
      ...
      bool isNeg = false;
      if(StringGetCharacter(intPart, 0) == '-') { isNeg = true; intPart = StringSubstr(intPart, 1); }
   ...
   string FormatValue(string type, double value)
     {
      if(type == "integer") return FormatWithSpaces(IntegerToString((long)MathRound(value)));
      if(type == "currency") return "$" + FormatWithSpaces(DoubleToString(value, 2));
      return FormatWithSpaces(DoubleToString(value, 2)); // Default fallback
     }

   string FormatKey(string key, double value)
     {
      string type = "decimal";
      for(int i = 0; i < m_count; i++)
         if(m_rules[i].key == key) { type = m_rules[i].type; break; }
      return FormatValue(type, value);
     }
  };

CMetricFormatter *g_Formatter;
   ...
   color ParseColorStr(string c)
     {
      ...
      if(low == "silver") return clrSilver;
      if(low == "slate") return clrSlateGray;
      if(low == "iron") return clrDimGray;
      if(low == "cobalt") return clrDodgerBlue;
      if(low == "indigo") return clrMediumSlateBlue;
      if(low == "white") return clrWhite;
      return clrWhite; // default
     }
  };
...
// Dynamic risk config from server (max 6 variables)
struct SRiskVar {
   string key;
   string label;
   bool   enabled;
...
// Per-magic floating PnL (for heartbeat enrichment)
long   g_FloatMagics[100];
double g_FloatValues[100];
int    g_FloatCount = 0;
...
// Live bar tracking is now handled natively by MT5 via CustomTicksAdd
...
bool g_AutoPadScale = true;    // Auto-scale padding toggle
...
public:
   ...
   ~CServerClient() {}
   bool   SendHeartbeat(long magic, double pnl, double dd, int trades, long &fMagics[], double &fValues[], int fCount);
...
bool CServerClient::SendHeartbeat(long magic, double pnl, double dd, int trades, long &fMagics[], double &fValues[], int fCount)
  {
   // Build floating_by_magic JSON map
   string floatMap = "";
   for(int i=0; i<fCount; i++)
     {
      if(i > 0) floatMap += ",";
      floatMap += "\"" + IntegerToString(fMagics[i]) + "\":" + DoubleToString(fValues[i], 2);
     }
...
void ParseRiskConfig(string resp)
  {
   ...
   // FOR PORTFOLIOS: Extract server-computed equity
   int peIdx = StringFind(resp, "\"portfolio_equity\":");
   if(peIdx >= 0 && g_ActiveMagic < 0)
     {
      int peS = peIdx + 19;
      int peE = StringFind(resp, ",", peS);
      if(peE < 0) peE = StringFind(resp, "}", peS);
      if(peE > peS)
        {
         g_StrategyEquity = StringToDouble(StringSubstr(resp, peS, peE - peS));
...
bool EnsurePnLSymbol()
  {
   if(!CustomSymbolCreate(g_PnlSymbol, "\\Custom"))
     {
      if(SymbolInfoInteger(g_PnlSymbol, SYMBOL_CUSTOM) != 1)
        { Print("[IR] Cannot create ", g_PnlSymbol); return false; }
     }
   CustomSymbolSetInteger(g_PnlSymbol, SYMBOL_DIGITS, 2);
   CustomSymbolSetString(g_PnlSymbol, SYMBOL_DESCRIPTION, "IronRisk Floating PnL");
   CustomSymbolSetString(g_PnlSymbol, SYMBOL_CURRENCY_PROFIT, "USD");
   CustomSymbolSetString(g_PnlSymbol, SYMBOL_CURRENCY_BASE, "USD");
   CustomSymbolSetString(g_PnlSymbol, SYMBOL_CURRENCY_MARGIN, "USD");
   CustomSymbolSetDouble(g_PnlSymbol, SYMBOL_POINT, 0.01);
   CustomSymbolSetDouble(g_PnlSymbol, SYMBOL_TRADE_TICK_SIZE, 0.01);
   CustomSymbolSetDouble(g_PnlSymbol, SYMBOL_TRADE_TICK_VALUE, 1.0);
   SymbolSelect(g_PnlSymbol, true);
   return true;
  }
...
void ClearSymbolHistory()
  {
   ...
   CustomRatesDelete(g_PnlSymbol, 0, TimeCurrent()+86400);
   CustomTicksDelete(g_PnlSymbol, 0, (long)(TimeCurrent()+86400)*1000);
  }

void ClearFloorSegments()
  {
   ObjectsDeleteAll(0, "IR_FS_");
   ObjectDelete(0, "IR_DDFLOOR"); // Wipe ghost line on switch
   ObjectDelete(0, "IR_PEAKL");   // Wipe live line on switch
   g_FloorSegCount = 0;
  }
...
void DrawTrailingFloor(MqlRates &rates[], int count)
  {
   ...
   // Start at the 0.00 floor inherently, rather than -999999.0 which breaks initialization
   double last_floor = 0.0 - g_MaxDrawdownLimit; 
   ...
   // Final segment with RAY_RIGHT (Always drawn to guarantee price label)
...
void FeedPnLUpdate(double val)
  {
   ...
   bool forceKeepalive = (now - lastTickTime >= 15);
   bool priceChanged = (val != g_LastFedPnL);
   
   // 1. Smart Tick Injection: Only feed CustomTicksAdd if price changes, 
   // or every 15s. This stops the "nervous tick" (1-second screen jitter) while 
   // keeping MT5's Custom Symbol engine alive so history bars don't vanish.
   // NOTE: We REMOVED CustomRatesUpdate because MT5 builds M1 bars natively 
   // from CustomTicksAdd. Manual replacement forces a massive chart flush!
   if(priceChanged || forceKeepalive || g_LastFedPnL == EMPTY_VALUE)
     {
      MqlTick t[1];
      ZeroMemory(t[0]);
      t[0].time     = now;
   ...
   if(rateCount > 0)
     {
      ...
      int written = CustomRatesReplace(g_PnlSymbol, 0, 
                                       rates[rateCount-1].time + 60, rates);
                                       
      // Initialize state for live tick injection
...
void DrawDashboard()
  {
   ...
   LB("IR_TITLE", 20, 12, "IRONRISK DASHBOARD  v58", 16, clrWhite);
   
   color sc = clrGray;
   string dispStatus = g_ServerStatus;
   if(g_ServerStatus=="NORMAL") { sc=clrLimeGreen; dispStatus="CONECTADO: IRONRISK CLOUD"; }
   else if(g_ServerStatus=="WARNING") { sc=clrOrange; dispStatus="ADVERTENCIA DE RIESGO"; }
   else if(g_ServerStatus=="CRITICAL") { sc=clrCrimson; dispStatus="RIESGO CRITICO"; }
   else if(g_ServerStatus=="KILL") { sc=clrRed; dispStatus="ERROR: TOKEN O CUENTA INVALIDA"; }
   else { dispStatus="ESPERANDO CONEXION..."; }
   ...
   LB("IR_PVAL", 32, 92, g_Formatter.FormatKey("current_pnl", g_StrategyEquity), 22, pc);
   LB("IR_TVAL", 32, 120, "Open: "+IntegerToString(g_LocalOpenTrades)+"  Closed: "+g_Formatter.FormatKey("total_trades", g_RealClosedTrades)+"  Float: "+g_Formatter.FormatKey("current_pnl", g_LocalFloatingPnL), 9, clrSilver);
   ...
   // Auto-scale pad toggle button
   ObjectCreate(0,"IR_AUTOPAD_BTN",OBJ_BUTTON,0,0,0);
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_XDISTANCE,246);
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_YDISTANCE,117);
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_XSIZE,24);
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_YSIZE,18);
   ObjectSetString(0,"IR_AUTOPAD_BTN",OBJPROP_TEXT,"A");
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_FONTSIZE,8);
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_BGCOLOR, g_AutoPadScale ? C'0,120,70' : C'50,50,50');
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_COLOR, g_AutoPadScale ? clrWhite : clrGray);
   ObjectSetString(0,"IR_AUTOPAD_BTN",OBJPROP_TOOLTIP,"Auto-Pad Dynamic Scale");
   ObjectSetInteger(0,"IR_AUTOPAD_BTN",OBJPROP_STATE,false);
    ...
    LB("IR_PEAK", 282, 94, "Peak:   " + g_Formatter.FormatKey("current_pnl", g_PeakPnL), 12, clrDodgerBlue);
    color ddc = (g_MaxDrawdownLimit>0 && g_StrategyEquity<=dd_floor) ? clrCrimson : clrOrange;
    string ddStr = g_MaxDrawdownLimit>0 ? g_Formatter.FormatKey("current_pnl", dd_floor) : "Sin limite";
       ...
       if(isMoney)
         {
          if(w.value_key == "current_pnl" || lim == 0)
             valStr = g_Formatter.FormatKey(w.value_key, current);
          else
             valStr = g_Formatter.FormatKey(w.value_key, current) + " / " + g_Formatter.FormatKey(w.value_key, lim) + pctStr;
          if(lim > 0 && current / lim > 0.8) valCol = clrOrangeRed;
          if(lim > 0 && current >= lim) valCol = clrCrimson;
         }
       else
         {
          if(lim > 0) valStr = g_Formatter.FormatKey(w.value_key, current) + " / " + g_Formatter.FormatKey(w.value_key, lim) + pctStr;
   ...
   for(int d=totalDeals-1; d>=0 && drawn<maxRows; d--)
     {
      ...
      // Volume
      double vol = HistoryDealGetDouble(tk, DEAL_VOLUME);
      LB("IR_TL_V_"+ri, c4, ry, g_Formatter.FormatKey("volume", vol), 8, clrSilver);
      ...
      LB("IR_TL_P_"+ri, c5, ry, g_Formatter.FormatKey("net_profit", pnl), 8, pnlClr);
   ...
   if(g_MaxDrawdownLimit > 0)
     {
      ...
      // Trailing DD floor line (peak - max tolerated DD from backend)
      double ddFloorLevel = g_PeakPnL - g_MaxDrawdownLimit;
      if(ObjectFind(0,"IR_PEAKL")<0)
        { ObjectCreate(0,"IR_PEAKL",OBJ_HLINE,0,0,ddFloorLevel); ObjectSetInteger(0,"IR_PEAKL",OBJPROP_COLOR,clrCrimson); ObjectSetInteger(0,"IR_PEAKL",OBJPROP_STYLE,STYLE_DOT); ObjectSetInteger(0,"IR_PEAKL",OBJPROP_WIDTH,1); ObjectSetInteger(0,"IR_PEAKL",OBJPROP_BACK,true); }
      else
        ObjectSetDouble(0,"IR_PEAKL",OBJPROP_PRICE,ddFloorLevel);
     }
  }
...
void UpdateChartScaling()
  {
   ...
   int first_vis = (int)ChartGetInteger(0, CHART_FIRST_VISIBLE_BAR);
   int vis_bars  = (int)ChartGetInteger(0, CHART_VISIBLE_BARS);
   if(vis_bars <= 0) return;
   
   int start_idx = first_vis - vis_bars + 1;
   if(start_idx < 0) start_idx = 0;
   int count = first_vis - start_idx + 1;
   if(count <= 0) return;
   
   double high_arr[];
   double low_arr[];
   if(CopyHigh(Symbol(), Period(), start_idx, count, high_arr) <= 0) return;
   if(CopyLow(Symbol(), Period(), start_idx, count, low_arr) <= 0) return;
   
   double actual_max = -99999999.0;
   double actual_min = 99999999.0;
   for(int i=0; i<ArraySize(high_arr); i++) { if(high_arr[i] > actual_max) actual_max = high_arr[i]; }
   for(int i=0; i<ArraySize(low_arr); i++)  { if(low_arr[i] < actual_min) actual_min = low_arr[i]; }
   
   // Include Trailing DD Floor in scaling if it is enabled and visible below price
   double floor_lvl = g_PeakPnL - g_MaxDrawdownLimit;
   if(g_MaxDrawdownLimit > 0 && floor_lvl < actual_min)
     {
      actual_min = floor_lvl;
     }
   
   if(actual_max < actual_min) return;
   double range = actual_max - actual_min;
   if(range == 0) range = 10.0;
   
   int total_px = (int)ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS);
   if(total_px <= 250) return;
   
   int top_pad_px = 240; // Dashboard height + margin
   int bot_pad_px = 30;  // Bottom margin
   int graph_px = total_px - top_pad_px - bot_pad_px;
   if(graph_px < 50) graph_px = 50;
   
   double price_per_px = range / (double)graph_px;
   double padded_max = actual_max + (price_per_px * top_pad_px);
   double padded_min = actual_min - (price_per_px * bot_pad_px);
   
   ChartSetDouble(0, CHART_FIXED_MAX, padded_max);
   ChartSetDouble(0, CHART_FIXED_MIN, padded_min);
  }

void CalculateLocalStats()
  {
   // === 1. Compute per-magic floating PnL for ALL open positions ===
   g_FloatCount = 0;
      ...
      if(PositionSelectByTicket(tk))
        {
         ...
         double posFloat = PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_SWAP);
         
         // Track per-magic floating
         bool found = false;
         for(int f=0; f<g_FloatCount; f++)
           {
            if(g_FloatMagics[f] == mag) { g_FloatValues[f] += posFloat; found = true; break; }
           }
         if(!found && g_FloatCount < 100)
           {
            g_FloatMagics[g_FloatCount] = mag;
            g_FloatValues[g_FloatCount] = posFloat;
            g_FloatCount++;
           }
   ...
   // === 2. Strategy Equity = cumulative closed P&L + current floating ===
   if(g_ActiveMagic >= 0)
     {
   ...
   // === 3. Recalculate cumulative closed PnL ===
   ...
   if(HistorySelect(0, TimeCurrent()))
     {
      ...
      if(closedCount >= g_RealClosedTrades || g_RealClosedTrades == 0)
        {
...
int OnInit()
  {
   ...
   g_IsDashboardMode = (StringFind(Symbol(), "IronRisk_PnL") == 0);
   if(g_IsDashboardMode)
     {
      g_PnlSymbol = Symbol(); // Adopt the specific custom symbol native to this chart!
     }
   else
     {
      // Bootstrap mode: Find an available Custom Symbol slot
      g_PnlSymbol = "IronRisk_PnL";
      int suffix = 2;
      while(SymbolInfoInteger(g_PnlSymbol, SYMBOL_CUSTOM) == 1 || SymbolInfoDouble(g_PnlSymbol, SYMBOL_BID) > 0)
        {
         g_PnlSymbol = "IronRisk_PnL_" + IntegerToString(suffix);
         suffix++;
         if(suffix > 100) break; // Sanity cap
        }
     }
   ...
   if(g_IsDashboardMode)
     {
      ...
      Print("[IR] Dashboard mode logically isolated to Custom Symbol: ", g_PnlSymbol);
      ...
      // Force instantaneous initial data fetch from backend BEFORE drawing UI
...
void OnDeinit(const int reason)
  {
   ...
   if(CheckPointer(g_Formatter)==POINTER_DYNAMIC) delete g_Formatter;
  }
...
void OnTimer()
  {
   ...
   // Sync with server FIRST, so we have fresh metrics
   if(CheckPointer(AppServer)==POINTER_DYNAMIC)
     {
      AppServer.SyncClosedDeals(g_ActiveMagic);
      AppServer.SendHeartbeat(g_ActiveMagic, g_StrategyEquity, g_LocalDrawdown, g_LocalOpenTrades, g_FloatMagics, g_FloatValues, g_FloatCount);
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
  {
   ...
   // Force network sync when user manually interacts with the EA UI
   // This guarantees the EA reports as "online" and refreshes limits immediately upon user click
   if(id == CHARTEVENT_OBJECT_CLICK && CheckPointer(AppServer) == POINTER_DYNAMIC)
     {
      CalculateLocalStats();
      AppServer.SendHeartbeat(g_ActiveMagic, g_StrategyEquity, g_LocalDrawdown, g_LocalOpenTrades, g_FloatMagics, g_FloatValues, g_FloatCount);
     }
   
   // Handle visual chart changes (e.g. scrolling/resizing)
   ...
   if(id==CHARTEVENT_OBJECT_CLICK)
     {
      ...
      if(sparam=="IR_AUTOPAD_BTN") { g_AutoPadScale=!g_AutoPadScale; ObjectSetInteger(0,sparam,OBJPROP_STATE,false); UpdateChartScaling(); DrawDashboard(); return; }
