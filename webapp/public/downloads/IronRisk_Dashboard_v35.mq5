...
void InitDefaultRiskVars()
  {
   g_RiskVarCount = 5;
   g_RiskVars[0].key="max_drawdown";      g_RiskVars[0].label="Max DD";          g_RiskVars[0].enabled=true;  g_RiskVars[0].limit=0; g_RiskVars[0].current=0; g_RiskVars[0].ctx_percentile=-1; g_RiskVars[0].ctx_label=""; g_RiskVars[0].ctx_color=clrGray;
...
bool CServerClient::SendHeartbeat(long magic, double pnl, double dd, int trades)
  {
   string json = "{\"api_token\":\""+m_token+"\",\"magic_number\":"+IntegerToString(magic)
      ...
      +",\"consecutive_losses\":"+IntegerToString(g_ConsecLosses)+",\"stagnation_days\":0,\"stagnation_trades\":0}";
...
void FeedPnLUpdate(double val)
  {
   ...
   // 1. Feed tick for live line display
   MqlTick t[1];
   ZeroMemory(t[0]);
   t[0].time     = now;
   ...
   // 2. Write/update current M1 bar (persistent on disk)
   datetime bar_time = now - (now % 60);
   
   if(bar_time != g_CurrentBarTime)
     {
      // New minute → new bar
   ...
   r[0].spread      = 0;
   r[0].real_volume = 0;
   CustomRatesReplace(PNL_SYMBOL, g_CurrentBarTime, g_CurrentBarTime + 60, r);
  }
...
void SeedHistoricalEquity()
  {
   ...
   if(rateCount > 0)
     {
      int written = CustomRatesReplace(PNL_SYMBOL, rates[0].time, 
...
void DrawDashboard()
  {
   ...
   LB("IR_TITLE", 20, 12, "IRONRISK DASHBOARD  v35", 16, clrWhite);
   color sc = clrGray;
   ...
   ObjectSetInteger(0,"IR_TLOG_BTN",OBJPROP_STATE,false);
    ...
    // === Dynamic risk variable cards with progress bars ===
    ObjectsDeleteAll(0, "IR_RV_");
     ...
     // Count enabled cards
     int enabledCount = 0;
     for(int ec=0; ec<g_RiskVarCount; ec++) if(g_RiskVars[ec].enabled) enabledCount++;
     ...
     for(int i=0; i<g_RiskVarCount; i++)
      {
       if(!g_RiskVars[i].enabled) continue;
        if(visIdx < g_RiskScrollOff) { visIdx++; continue; }
        if(drawn >= availCards) { visIdx++; continue; }
       // cardW_r defined above as cardW_r
       // cardH = 70
       string pfx = "IR_RV_" + IntegerToString(i);
       PNL(pfx+"_BG", rvX, 68, cardW_r, 70, C'25,25,25');
       LB(pfx+"_LBL", rvX+10, 72, g_RiskVars[i].label, 8, clrGray);
        // Value text: current / limit (always computed)
        double current = g_RiskVars[i].current;
        string valStr = "";
        color valCol = clrWhite;
        
        bool hasCtx = (g_RiskVars[i].ctx_label != "" || g_RiskVars[i].ctx_percentile >= 0);
        if(g_RiskVars[i].key=="max_drawdown" || g_RiskVars[i].key=="daily_loss")
          {
           valStr = DoubleToString(current, 2) + " / " + DoubleToString(g_RiskVars[i].limit, 2) + " $";
           if(g_RiskVars[i].limit > 0 && current / g_RiskVars[i].limit > 0.8) valCol = clrOrangeRed;
           if(g_RiskVars[i].limit > 0 && current >= g_RiskVars[i].limit) valCol = clrCrimson;
          }
        else
          {
           int cur_int = (int)MathRound(current);
           int lim_int = (int)MathRound(g_RiskVars[i].limit);
           valStr = IntegerToString(cur_int) + " / " + IntegerToString(lim_int);
          }
        // Numérico siempre en Y=86, font 11 (un poco más pequeño para caber)
        LB(pfx+"_VAL", rvX+10, 86, valStr, 11, valCol);
        
        // Etiqueta estadística ("Expected range" etc.) en Y=104
        if(hasCtx && g_RiskVars[i].ctx_label != "")
          {
           string ctxTxt = g_RiskVars[i].ctx_label;
           if(StringLen(ctxTxt) > 28) ctxTxt = StringSubstr(ctxTxt, 0, 26) + "..";
           LB(pfx+"_CTX", rvX+10, 104, ctxTxt, 9, g_RiskVars[i].ctx_color);
          }
          
        // Progress bar y=118 (shift down para dar espacio)
       double lim = g_RiskVars[i].limit;
       double pct = (lim > 0) ? MathMin(current / lim, 1.0) : 0.0;
       int barW = cardW_r - 20;
       int barH = 8;
       int barX = rvX + 10;
       int barY = 118; // Fijo 118 para alejarlo del txt contextual
       // Bar background
       PNL(pfx+"_BAR_BG", barX, barY, barW, barH, C'40,40,40');
       // Bar fill with dynamic color
       color barCol = clrLimeGreen;
       if(hasCtx) 
         {
          barCol = g_RiskVars[i].ctx_color;
          if(g_RiskVars[i].ctx_percentile >= 0 && g_RiskVars[i].ctx_percentile <= 100)
             pct = g_RiskVars[i].ctx_percentile / 100.0;
         }
       else
         {
       ...
       if(fillW > 0)
          PNL(pfx+"_BAR_FILL", barX, barY, fillW, barH, barCol);
          
       // Percentage text
       string pctStr = "";
       if(hasCtx && g_RiskVars[i].ctx_percentile >= 0) pctStr = "P" + IntegerToString(g_RiskVars[i].ctx_percentile);
       else pctStr = DoubleToString(pct * 100, 0) + "%";
       color pctCol = clrGray;
       if(hasCtx) pctCol = g_RiskVars[i].ctx_color;
       else if(pct > 0.7) pctCol = clrWhite;
       LB(pfx+"_PCT", rvX + cardW_r - 12, 73, pctStr, 9, pctCol, ANCHOR_RIGHT_UPPER);
       // Eye toggle for floor lines
       if(g_RiskVars[i].key=="max_drawdown")
         {
          string eyeName = "IR_EYE_" + IntegerToString(i);
          ObjectCreate(0, eyeName, OBJ_BUTTON, 0, 0, 0);
          ObjectSetInteger(0, eyeName, OBJPROP_XDISTANCE, rvX + cardW_r - 25);
          ObjectSetInteger(0, eyeName, OBJPROP_YDISTANCE, 70);
          ObjectSetInteger(0, eyeName, OBJPROP_XSIZE, 20);
          ObjectSetInteger(0, eyeName, OBJPROP_YSIZE, 16);
          ObjectSetInteger(0, eyeName, OBJPROP_FONTSIZE, 8);
          ObjectSetInteger(0, eyeName, OBJPROP_BGCOLOR, g_ShowFloorLine ? C'60,60,60' : C'30,30,30');
          ObjectSetInteger(0, eyeName, OBJPROP_COLOR, g_ShowFloorLine ? clrWhite : clrGray);
          ObjectSetString(0, eyeName, OBJPROP_TEXT, g_ShowFloorLine ? "O" : "-");
...
void CalculateLocalStats()
  {
   ...
   // Closed today (for display)
   g_LocalClosedToday = 0.0;
   datetime day = TimeCurrent()-(TimeCurrent()%86400);
   if(HistorySelect(day,TimeCurrent()))
     { for(int d=0;d<HistoryDealsTotal();d++) { ulong tk=HistoryDealGetTicket(d); if(tk>0){ long m=HistoryDealGetInteger(tk,DEAL_MAGIC); if(g_ActiveMagic==0||g_ActiveMagic==m){ long en=HistoryDealGetInteger(tk,DEAL_ENTRY); if(en==DEAL_ENTRY_OUT||en==DEAL_ENTRY_INOUT) g_LocalClosedToday+=HistoryDealGetDouble(tk,DEAL_PROFIT)+HistoryDealGetDouble(tk,DEAL_SWAP)+HistoryDealGetDouble(tk,DEAL_COMMISSION);}}} }
  
    // Count consecutive losses from deal history (for risk card)
    g_ConsecLosses = 0;
    if(HistorySelect(0, TimeCurrent()))
      {
       for(int dc=HistoryDealsTotal()-1; dc>=0; dc--)
         {
          ulong dtk=HistoryDealGetTicket(dc);
          if(dtk<=0) continue;
          long dm=HistoryDealGetInteger(dtk,DEAL_MAGIC);
          if(g_ActiveMagic!=0 && dm!=g_ActiveMagic) continue;
          long den=HistoryDealGetInteger(dtk,DEAL_ENTRY);
          if(den!=DEAL_ENTRY_OUT && den!=DEAL_ENTRY_INOUT) continue;
          double dpnl=HistoryDealGetDouble(dtk,DEAL_PROFIT)+HistoryDealGetDouble(dtk,DEAL_SWAP)+HistoryDealGetDouble(dtk,DEAL_COMMISSION);
          if(dpnl<0) g_ConsecLosses++;
          else break; // First win stops the count
         }
      }
}
   ...
   if(g_IsDashboardMode)
     {
      ...
      // Style: clean green line, dark background
      ChartSetInteger(0, CHART_MODE, CHART_LINE);
      ChartSetInteger(0, CHART_COLOR_CHART_LINE, clrLimeGreen);
      ...
      if(ci>=0 && ci<g_TotalStrategies)
        {
         ...
         // Heartbeat FIRST to get new risk_config
         if(CheckPointer(AppServer)==POINTER_DYNAMIC)
            AppServer.SendHeartbeat(g_ActiveMagic, 0.0, 0.0, 0);
         // THEN seed with correct MaxDrawdownLimit
         CalculateLocalStats();
         SeedHistoricalEquity();
         FeedPnLUpdate(g_StrategyEquity);
         DrawDashboard();
