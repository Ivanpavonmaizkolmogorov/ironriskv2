...
void DrawDashboard()
  {
   ...
   LB("IR_TITLE", 20, 12, "IRONRISK DASHBOARD  v51", 16, clrWhite);
   color sc = clrGray;
...
int OnInit()
  {
   ...
   // --- SINGLETON LOCK ---
   // Protects against users opening the EA on multiple tabs and causing Custom Symbol database corruption
   long existingChart = 0;
   if(GlobalVariableCheck("IR_DASHBOARD_CHART"))
      existingChart = (long)GlobalVariableGet("IR_DASHBOARD_CHART");
      
   if(existingChart != 0 && existingChart != ChartID())
     {
      // Check if the other chart is really alive in this terminal session
      if(ChartSymbol(existingChart) != "")
        {
         Alert("⚠️ PROTECCIÓN IRONRISK: ¡Ya tienes otro Dashboard ejecutándose en otra pestaña! (", ChartSymbol(existingChart), "). Cierra la otra pestaña o bórralo de ahí primero para evitar conflictos. Auto-cerrando...");
         ExpertRemove();
         return(INIT_FAILED);
        }
     }
   // We are clear to take the lock
   GlobalVariableSet("IR_DASHBOARD_CHART", (double)ChartID());
...
void OnDeinit(const int reason)
  {
   if(GlobalVariableCheck("IR_DASHBOARD_CHART"))
     {
      long lockCid = (long)GlobalVariableGet("IR_DASHBOARD_CHART");
      if(lockCid == ChartID()) GlobalVariableDel("IR_DASHBOARD_CHART");
     }
