...
void DrawDashboard()
  {
   ...
   LB("IR_TITLE", 20, 12, "IRONRISK DASHBOARD  v53", 16, clrWhite);
   color sc = clrGray;
...
void DrawBootstrapScreen()
  {
   ...
   LB("IR_BOOT_T1", 80, 100, "IRONRISK DASHBOARD v53", 20, clrWhite);
      ...
      if(ci>=0 && ci<g_TotalStrategies)
        {
         ...
         // 2. Fetch fresh config immediately so risk cards update instantly
          if(CheckPointer(AppServer)==POINTER_DYNAMIC)
            {
             long emptyMag[]; double emptyVal[];
             AppServer.SendHeartbeat(g_ActiveMagic, 0.0, 0.0, 0, emptyMag, emptyVal, 0);
            }
         CalculateLocalStats();
         SeedHistoricalEquity();
         FeedPnLUpdate(g_StrategyEquity);
